...
This document describes the upgrade procedure of a `tichexd` full-node to a new version. 

## Software Upgrade

First, stop your instance of `tichexd`. Next, upgrade the software:
...
If you are joining the mainnet, fetch the genesis from the [mainnet repo](coming later). If you are joining a public testnet, fetch the genesis from the appropriate testnet in the [testnet repo](coming soon). Otherwise, fetch it from your trusted source. 

Save the new genesis as `new_genesis.json`. Then replace the old `genesis.json` with `new_genesis.json`

```bash
cd $HOME/.tichexd/config
...
```bash
cd $HOME/.tichexd/config
tichexd export --for-zero-height --height=<export-height> > new_genesis.json
```
...
::: warning 
If you are running a **validator node** on the mainnet, always be careful when doing `tichexd unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
